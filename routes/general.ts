...
router.post('/flights/checkin', async (req: Request, res: Response) => {
    ...
    try {
        ...
        let seat_available = (await client.query(queryCheckSeat)).rows.length
        ...
        let ticket = (await client.query(queryFetchTicket)).rows
        if (ticket.length === 0) {
            res.statusCode = 500
        ...
        let dopPrice: any[] | number = (await client.query(queryBoardingPrice)).rows
        ...
        let stockPrice: any[] | number = (await client.query(queryStockPrice)).rows
        ...
        const generateBoardingID = () => {
            return crypto.randomInt(0, 1000)
        }
        ...
        while (1) {
            let reslen = (await client.query(queryCheckBoardingID)).rows.length
            if (reslen !== 0)
        ...
        const insBoardingPassRes = client.query(queryInsertBoardingPass)
