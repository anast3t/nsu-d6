...
function time2Date(departure, arrival) {
    let date = new Date();
...
router.get('/', async (req, res) => {
    ...
    let results = [];
    try {
        results = ((await db_1.db.query(queryAll)).rows);
        results.forEach(el => {
            el.arrival_utc = el.arrival;
            el.departure_utc = el.departure;
            [el.departure, el.arrival] = time2Date(el.departure, el.arrival);
        });
        // res.json(results)
    }
    catch (err) {
    ...
    let uniqueAirports = [...new Set(uniqueFlights.map(el => {
    ...
    for (let i = 0; i < uniqueAirports.length; i++) {
        ...
        try {
            let airport = (await db_1.db.query(query)).rows[0];
...
router.post('/booking', async (req, res) => {
    const { passenger_name, contact_data, fare_condition, route, date } = req.body;
    function returnRouteWhere(indent) {
        let str = ' (';
    ...
    let flights = (await db_1.db.query(queryFlights)).rows;
    ...
    let seats = (await db_1.db.query(queryAllSeats)).rows;
    ...
    let prices = (await db_1.db.query(queryPrices)).rows;
    ...
    let booked = (await db_1.db.query(queryBooked)).rows;
    booked.forEach(el => {
    ...
    while (1) {
        let reslen = (await db_1.db.query(queryCheckId)).rows.length;
        if (reslen !== 0)
    ...
    while (1) {
        let reslen = (await db_1.db.query(queryCheckUserId)).rows.length;
        if (reslen !== 0)
    ...
    while (1) {
        let reslen = (await db_1.db.query(queryCheckTicketId)).rows.length;
        if (reslen !== 0)
